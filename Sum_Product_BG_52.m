...
[B,Hfull,z] = nrldpc_Hmatrix(baseGraph5GNR,52); % Convert the base H matrix to binary H matrix with z=52
...
nBlockLength_vec = zeros(length(coderate), 1); % To store block lengths for normal approximation
...
for cr = coderate
    ...
    [mb,nb] = size(B); % mb and nb depend on NR_2_6_52
    kb = nb - mb; % Number of information columns
    kNumInfoBits = kb * z; % Number of information bits

    k_pc = kb-2; % Systematic bits excluding 2 punctured bits
    nbRM = ceil(k_pc/cr); % Number of coded bits per z, adjusted for code rate
    nBlockLength = nbRM * z; % Number of encoded bits
    nBlockLength_vec(cr_idx) = nBlockLength; % Store for normal approximation
...
function [B,H,z] = nrldpc_Hmatrix(BG,z)
    % Load the base matrix NR_2_6_52
    % Note: The actual loading of the matrix file depends on your file structure
    % Here, we assume NR_2_6_52 is available as a variable or file
